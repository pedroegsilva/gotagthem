module github.com/pedroegsilva/gofindrules

go 1.17

require github.com/pedroegsilva/gofindthem v0.2.1-0.20210926214542-6f26d2559b8c

require (
	github.com/anknown/ahocorasick v0.0.0-20190904063843-d75dbd5169c0 // indirect
	github.com/anknown/darts v0.0.0-20151216065714-83ff685239e6 // indirect
	github.com/cloudflare/ahocorasick v0.0.0-20210425175752-730270c3e184 // indirect
	github.com/petar-dambovaliev/aho-corasick v0.0.0-20210512121028-af76a9ff7276 // indirect
)
